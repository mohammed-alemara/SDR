  ...
  while (stop==0) { 
   ...
   # Second step: calculate beta when given a, b 
    ...
    for(j in 1:n) { 
      ...
      #######################
      #shrinkage by use MCP #     
      #######################  

      scad.cv <- cv.ncvreg(d1[, -1], d1[, 1],penalty="MCP", dfmax=1000,max.iter=10^4)
      lambda.cv.scad<- scad.cv$lambda.min
      scad.coeff<- ncvreg(d1[, -1], d1[, 1], penalty='MCP',dfmax=1000,lambda=lambda.cv.scad)$beta[-1,1]
      coeff<-scad.coeff
