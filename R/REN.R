...
ind_rEN= function(x,ind2,B,y,X){
  p.g = length(x)
  if(p.g>1){
    ress = crossprod(y-X[,ind2]%*%x)
    betastar = (ress) + sum(B/(abs(x)))  + sum(B*(x)^2)
    int = sqrt(1+B)*betastar
    
  }else{
    if(p.g==1){
      ress = crossprod(y-X[,ind2]*x)
      betastar = (ress) + (B/abs(x)) + (B*(x)^2)
      int = sqrt(1+B)*betastar
    }
  }
  return(int)
...
ind_fun_rEN=function(ind2,B,y,X,tau,a0,b0,p){ 
  a0=0.01;b0=0.01
  tau = 1
  p.g=length(ind2)
  ...
  if(p.g >1){
    ...
    initial_x = fit 
    wrapper <- function(theta) ind_rEN(theta,ind2,B,y,X)
