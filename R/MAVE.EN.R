  ...
  while (stop==0) { 
   ...
   # Second step: calculate beta when given a, b 
    ...
    for(j in 1:n) { 
      ...
      ###############################
      #shrinkage by use Elastic net #     
      ###############################   
      
      penalty.cv    <- cv.glmnet(d1[, -1], d1[, 1], alpha = 0.5)
      cv      <- penalty.cv$lambda.min
      model.ElasticNet<-glmnet(d1[ ,-1],d1[ ,1],alpha=0.5,scale=1, lambda=cv) 
