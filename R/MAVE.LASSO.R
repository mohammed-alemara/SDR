...
for (q in 1:d) { #q=1
  ...
  while (stop==0) { 
   ...
   # Second step: calculate beta when given a, b 
    ...
    for(j in 1:n) { 
      ...
      #########################
      #shrinkage by use lasso #     
      #########################   
      
      penalty.cv    <- cv.glmnet(d1[, -1], d1[, 1], alpha = 1)
      cv      <- penalty.cv$lambda.min
      model.ElasticNet<-glmnet(d1[ ,-1],d1[ ,1],alpha=1,scale=1, lambda=cv) 
 ...
 betaold<-orthonormalization(betaold, basis=F, norm=T)
} 
